...
const clockIn = async (employeeId) => {
    ...
    const lateMinutes = calculateLateMinutes(now); // (= 0) if he not late
...
const getOverview = async () => {
    ...
    return {
        ...
        ...counts,
        notArrived: totalActive - counts.present - counts.late - counts.absent - counts.on_leave ,
    };
};
...
const markAbsentees = async () => {
    ...
    const existing = await Attendance.find({ date: today, }).distinct("employeeId"); // don't repeat employee
    ...
    const absentRecords = activeEmployees.filter(employee => !existing.has(employee._id.toString())) // get the absent employee
                                         .map(employee => ({
    ...
    if(absentRecords.length > 0) {
        await Attendance.insertMany(absentRecords, { ordered: false }); // don't stop the insert if error in one record
    }
